...
### 3. Timeline ธุรกรรมการเงิน

**บังคับใส่ถ้ามีธุรกรรม ≥ 3 รายการที่ระบุวันที่ได้** ถ้าน้อยกว่านั้น **ข้ามหัวข้อทั้งหมดนี้ไปเลย** (ห้ามใส่แม้แต่ heading) — ตารางในข้อ 2 พอแล้ว

**ส่วนนี้แบ่งเป็น 2 ก้อนย่อย รวมอยู่ภายใต้หัวข้อ "3. Timeline ธุรกรรมการเงิน" เดียว** (ห้ามแยกเป็นหัวข้อ "4. Flowchart" — ทุกอย่าง visual อยู่รวมในข้อ 3)

#### 3.1 Timeline text-art (บังคับ — render ได้ทุก format)

**ใช้รูปแบบนี้เป๊ะๆ — บรรทัดเหตุการณ์ + บรรทัดเปล่า + ลูกศร ↓ + บรรทัดเปล่า + บรรทัดเหตุการณ์ถัดไป** (ห้ามใช้อักขระวาดเส้น `│` `─` `┃` ฯลฯ เพราะ font ใน PDF บางตัวไม่รองรับและทำให้ render พัง)

```
📅 dd/mm/yyyy  ➜  เหตุการณ์ทางการเงิน 1  (฿ จำนวน)

       ↓

📅 dd/mm/yyyy  ➜  เหตุการณ์ทางการเงิน 2  (฿ จำนวน)

       ↓

📅 dd/mm/yyyy  ➜  เหตุการณ์ทางการเงิน 3  ⚡ จุดวิกฤต  (฿ จำนวน)

       ↓

📅 dd/mm/yyyy  ➜  เหตุการณ์ปัจจุบัน
```

**กฎ Timeline:**
- แต่ละโหนดต้องมี **วันที่ + เหตุการณ์ + จำนวนเงิน**
- เรียงจากเก่าไปใหม่
- ใส่ ⚡ ที่จุดวิกฤต (ผิดนัด / โอนต้องสงสัย / ฟ้อง)
- ขั้นต่ำ 3 โหนด สูงสุด 8 โหนด
- **ห้ามใช้** อักขระวาดเส้น box-drawing (`│` `─` `┃` `└` `┌` ฯลฯ) — ใช้แค่ `↓` `➜` `📅` `⚡` ที่ระบุข้างบนเท่านั้น

#### 3.2 Flowchart Mermaid (Optional — ใส่เฉพาะมี branching จริง)

**ใส่ก้อน Mermaid นี้ทันทีหลัง Timeline ด้านบน เฉพาะถ้าธุรกรรมมีเงื่อนไข/แตกสาขาจริง** เช่น "ถ้าชำระตรงเวลา → A, ถ้าไม่ → B"

**ถ้าธุรกรรมเป็นเส้นตรงไม่มีสาขา — อย่าใส่ส่วนนี้เลย** (Timeline 3.1 เพียงพอ ไม่ต้องใส่ heading "3.2" ด้วย)
...
**กฎ Mermaid:**
- ใส่จำนวนเงินจริงในทุกโหนด
- ใช้ `{{...}}` สำหรับโหนดคำถาม (เงื่อนไข)
- ใช้ `[...]` สำหรับโหนดเหตุการณ์ทั่วไป
- ตัวอักษรในโหนดต้องครอบด้วย `"..."` ถ้ามีอักขระพิเศษ

== สิ่งที่ห้ามทำ ==
❌ ห้ามใส่หัวข้ออื่น เช่น ข้อกฎหมาย, คู่ความ, ทุนทรัพย์, จุดแข็ง/อ่อน, กลยุทธ์ — ผู้ใช้จะดูบทวิเคราะห์เต็มต่างหาก
❌ ห้ามเขียนเป็นย่อหน้ายาว — ทุกอย่างต้องอยู่ใน 3 ส่วน: สรุปย่อ + ตาราง + Timeline (พร้อม Mermaid ถ้าเหมาะ)
❌ ห้ามตีความกฎหมาย — เน้นเฉพาะ "เงินไหลอย่างไร"
❌ **ห้ามสร้างหัวข้อ "4. Flowchart" หรือ "4. Flowchart (Mermaid)" แยก** — Mermaid ต้องอยู่ใน section 3 เสมอ (3.2)
❌ **ห้ามใส่ Mermaid block ถ้าไม่มี branching จริง** — ข้ามไปเลย ไม่ต้องใส่ heading 3.2 ด้วย
❌ **ห้ามใส่ Timeline (section 3 ทั้งก้อน) ถ้ามีธุรกรรมน้อยกว่า 3 รายการ** — ตารางในข้อ 2 พอแล้ว
